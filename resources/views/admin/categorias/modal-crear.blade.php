...
<div id="modalCrearCategoria" class="hidden fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-all duration-300">
    <div class="fixed inset-0 bg-transparent" onclick="closeCreateModal()"></div>
    
    <div id="createContainer" class="modal-container relative rounded-[24px] w-full max-w-md mx-auto shadow-2xl scale-95 opacity-0 transition-all duration-200 flex flex-col overflow-hidden max-h-[92vh]" style="background-color: var(--card-color); border: 1px solid var(--border-color);">

        {{-- Encabezado del Modal --}}
        <div class="flex items-center gap-4 p-5 sm:p-6 pb-5 shrink-0" style="background-color: var(--card-color);">
            <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-[#b74309]/10 border border-[#b74309]/20 flex items-center justify-center shrink-0">
                <i class="fas fa-plus text-[#b74309] text-lg"></i>
            </div>
            <div>
                <h2 class="text-[17px] sm:text-[19px] font-black tracking-tight leading-tight" style="color: var(--text-color);">Nueva Categoría</h2>
                <p class="text-[10px] font-bold uppercase tracking-widest mt-1" style="color: var(--text-muted);">Agregar al menú</p>
            </div>
            <button type="button" onclick="closeCreateModal()" class="ml-auto w-9 h-9 flex items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/5 active:scale-95 transition-colors outline-none shrink-0" style="color: var(--text-muted);">
        ...
        <form action="{{ route('admin.categorias.store') }}" method="POST" class="px-5 sm:px-6 py-6 space-y-6 overflow-y-auto">
            ...
            <div class="space-y-2" id="dropdownAreaCrearContainer" x-data="{
                ...
                <div class="relative">
                    <button type="button" 
                        @click="open = !open" 
                        onclick="toggleDropdownCrearArea(event)"
                        ...
                        <i id="icon_crear_area_chevron" class="fas fa-chevron-down text-xs transition-transform duration-200" :class="{ 'rotate-180': open }" style="color: var(--text-muted);"></i>
                    </button>

                    <div id="menu_crear_area_impresion" 
                        x-show="open" 
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3 w-full h-12 rounded-2xl px-3" style="background-color: var(--input-bg); border: 1px solid var(--border-color);">
                    <input type="color" name="color" value="#b74309" class="w-8 h-8 rounded-xl cursor-pointer border-0 bg-transparent outline-none">
